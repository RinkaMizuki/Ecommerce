  ...
  const handleFilter = async (sale = "", priceRange = {}, sortList = []) => {
    try {
      ...
      if (
        ...
        filterObj.priceRange = [priceRange.priceLeft, priceRange.priceRight];
      }
      ...
      const resRange = res.headers["content-range"]
        ...
        .split("/")[0]
        .split("-");
      const resTotal = res.headers["content-range"].split(" ")[1].split("/")[1];
  ...
  return (
    <div className={cx("flashsale-container")} ref={scrollRef}>
        ...
        if (p.id == productId) {
          return (
            <div style={{ fontFamily: "Poppins" }} key={p.id}>
              <Lightbox onClose={closeLightBox} url={p.url} alt={p.title} />
            </div>
          );
        }
        return null;
      ...
      <div className={cx("banner-wrapper")}>
        ...
        />
        {typeSale === "flashsale" && <CountDown className={cx("count-down")} />}
      </div>
