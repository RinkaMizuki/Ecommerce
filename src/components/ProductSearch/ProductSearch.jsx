  ...
  const handleShowMoreProduct = async () => {
    try {
      const res = await getCategoryById(
        `/Admin/categories/${products[0]?.categoryId}`
      );
      ...
      {products?.length ? (
        products.map((p, index) => (
          <Fragment key={p.id}>
                    ...
                    >
                      {p?.price ? (
                        <>
                          ...
                          <span className={cx("product-price-real")}>
            ...
            {index === SHOW_PRODUCT_MAX && (
              <span
                className={cx("product-showmore")}
        ...
        <span className={cx("product-unavaiable")}>
